...
RINGER_SWITCH_T RINGER_SWITCH = RINGER_ON;
uint32_t window_accumulation = 0;
const int DFC_ACCUMULATION_SIZE = 8;
uint64_t dfc_accumulation = 0;
const int NUM_RANDOM_MULTIPLIERS = 100;
uint16_t random_multiplier[NUM_RANDOM_MULTIPLIERS];
int random_multipler_idx = 0;

//#include "ringer_window_result.h"

//const uint8_t DFC_WINDOW_MATCH = 0x00;
//const uint8_t DFC_WINDOW_MISMATCH = 0xff;
//
//const uint32_t RINGER_RESULT_SIZE = 50000;

//RingerWindowResult window_result;
//struct window_result_t {
//  inline void append(Byte byte) {
//    acc_id |= byte;
//  }
//
//  inline void append(uint32_t data, int size) {
//    switch (size) {
//      case 2:
//        acc_byte[0] |= data >> 24 & 0xFF;
//        acc_byte[1] |= data >> 16 & 0xFF;
//        break;
//      case 4:
//        acc_byte[0] |= data >> 24 & 0xFF;
//        acc_byte[1] |= data >> 16 & 0xFF;
//        acc_byte[2] |= data >> 8 & 0xFF;
//        acc_byte[3] |= data & 0xFF;
//        break;
//      default:
//        assert(false);
//    }
...
//  Byte acc_byte[4];
//} window_result;
//#define MAX_DFC_SIZE 50000
//Byte dfc_result[MAX_DFC_SIZE] = { 0 };
//int dfc_size = 10;

enum DFC_COMPONENT_ID {DFCID_DirectFilter,
                       DFCID_cDF0, DFCID_cDF1, DFCID_cDF2,
                       DFCID_ADD_DF_4_plus, DFCID_ADD_DF_4_1,
                       DFCID_ADD_DF_8_1, DFCID_ADD_DF_8_2,
                       DFCID_CompactTable1, DFCID_CompactTable2,
                       DFCID_CompactTable4, DFCID_CompactTable8};

inline uint32_t _log2(uint32_t x) {
  uint32_t log = 0;
  while (x >>= 1)
    ++log;
  return log;
...
//#define recombine(index, mask) (index << 3 | _log2(mask))
#define recombine(index, mask) (index | mask)
HMAC RingerHMAC;

//const size_t NumRandomMultiplier = 100;
//BigInt RandomMultiplier[NumRandomMultiplier];
//void InitRandomMultiplier() {
//  for(size_t i = 0; i < NumRandomMultiplier; ++i)
//    RandomMultiplier[i] = rand() % 0x100000000;
//}
...
DFC_STRUCTURE * DFC_New (void)
{
    ...
    if (p)
    {
        ...
        p->init_hash = reinterpret_cast<DFC_PATTERN **>(malloc(sizeof(DFC_PATTERN *) * INIT_HASH_SIZE));
        if(p->init_hash == NULL){
            exit(1);
        }
...
static int Verification_CT1(VERIFI_ARGUMENT)
{
  if (RINGER_SWITCH == RINGER_ON) {
    //window_result.append(DFCID_CompactTable1);
    //window_result.append(*(buf - 2));
    window_accumulation |= DFCID_CompactTable1;
    window_accumulation |= *(buf - 2);
  }
	int i;
...
static int Verification_CT2(VERIFI_ARGUMENT)
{
	...
	crc &= CT2_TABLE_SIZE_MASK;
  if (RINGER_SWITCH == RINGER_ON) {
    //window_result.append(DFCID_CompactTable2);
    //window_result.append(crc, 2);
    window_accumulation |= DFCID_CompactTable2;
    window_accumulation |= crc;
  }
...
static int Verification_CT4_7(VERIFI_ARGUMENT)
{
	...
	crc &= CT4_TABLE_SIZE_MASK;
  if (RINGER_SWITCH == RINGER_ON) {
    //window_result.append(DFCID_CompactTable4);
    //window_result.append(crc, 4);
    window_accumulation |= DFCID_CompactTable4;
    window_accumulation |= crc;
  }
...
static int Verification_CT8_plus(VERIFI_ARGUMENT)
{
	...
	crc &= CT8_TABLE_SIZE_MASK;
  if (RINGER_SWITCH == RINGER_ON) {
    //window_result.append(DFCID_CompactTable8);
    //window_result.append(crc, 4);
    window_accumulation |= DFCID_CompactTable8;
    window_accumulation |= crc;
  }
...
static inline int Progressive_Filtering(PROGRE_ARGUMENT) 
{
  ...
  if (RINGER_SWITCH == RINGER_ON) {
    //window_result.append(DFCID_cDF0);
    //window_result.append(dfc->cDF0[*(buf - 2)]);
    window_accumulation |= DFCID_cDF0;
    window_accumulation |= dfc->cDF0[*(buf - 2)];
  }
    if(dfc->cDF0[*(buf-2)]){
		matches = Verification_CT1(VERIFI_PARAMETER);
	}
    if (RINGER_SWITCH == RINGER_ON) {
      //window_result.append(DFCID_cDF1);
      //window_result.append(recombine(idx, dfc->cDF1[idx]), 2);
      window_accumulation |= DFCID_cDF1;
      window_accumulation |= recombine(idx, dfc->cDF1[idx]);
    }
    ...
    if(rest_len >= 4){
        ...
        if (RINGER_SWITCH == RINGER_ON) {
          //window_result.append(DFCID_ADD_DF_4_plus);
          //window_result.append(recombine(index, dfc->ADD_DF_4_plus[index]), 2);
          window_accumulation |= DFCID_ADD_DF_4_plus;
          window_accumulation |= recombine(index, dfc->ADD_DF_4_plus[index]);
        }
        if (unlikely((mask & dfc->ADD_DF_4_plus[index]))) {
          if (RINGER_SWITCH == RINGER_ON) {
            //window_result.append(DFCID_ADD_DF_4_1);
            //window_result.append(recombine(index, dfc->ADD_DF_4_1[index]), 2);
            window_accumulation |= DFCID_ADD_DF_4_1;
            window_accumulation |= recombine(index, dfc->ADD_DF_4_1[index]);
          }
...
int DFC_Search(SEARCH_ARGUMENT)
{
  ...
  if (RINGER_SWITCH == RINGER_ON) {
    ...
    if (RINGER_SWITCH == RINGER_ON) {
      ...
      //window_result.clear();
      //window_result.append(DFCID_DirectFilter);
      //window_result.append(recombine(index, DirectFilter1[index]), 2);
      window_accumulation |= recombine(index, DirectFilter1[index]);
    }
		//matches++;
    ...
    //if (RINGER_SWITCH == RINGER_ON) {
    //  //dfc_result.insert(dfc_result.end(), window_result.begin(), window_result.end());
    //  memcpy(&dfc_result[dfc_size], window_result.data(), window_result.size());
    //  dfc_size += window_result.size();
    //}

	} /* for (i=0; i<buflen; i++) */

  // accumulate window result to dfc result
  if (RINGER_SWITCH == RINGER_ON) {
    //dfc_result.push_back(DFCID_CompactTable1);
    //dfc_result.push_back(buf[buflen - 1]);
    /*dfc_result[dfc_size++] = DFCID_CompactTable1;
    dfc_result[dfc_size++] = buf[buflen - 1];*/
    dfc_accumulation += window_accumulation*random_multiplier[random_multipler_idx++%NUM_RANDOM_MULTIPLIERS];
  }
    ...
    if (RINGER_SWITCH == RINGER_ON) {
      RingerHMAC((Byte*)&dfc_accumulation, DFC_ACCUMULATION_SIZE, ringer);
    }
